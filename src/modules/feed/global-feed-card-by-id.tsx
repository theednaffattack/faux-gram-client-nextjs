                ...
                onSubmit={async (data, { setErrors, resetForm }) => {
                  addCommentToPost({
                    ...
                    update: (cache, { data }) => {
                      ...
                      let fromCache = cache.readQuery<GetGlobalPostByIdQuery>({
                      ...
                      if (
                        fromCache &&
                        fromCache.getGlobalPostById &&
                        fromCache.getGlobalPostById.comments
                      ) {
                        cache.writeQuery<GetGlobalPostByIdQuery>({
                          query: GetGlobalPostByIdDocument,
                          data: {
                            getGlobalPostById: {
                              ...fromCache.getGlobalPostById,
                              ...
                              comments:
                                fromCache.getGlobalPostById.comments &&
                                fromCache.getGlobalPostById.comments.concat(
...
export const FeedCard: React.FunctionComponent<ISingleFeedCardProps> = ({
  ...
  const globalPostsUpdateFunction: MutationUpdateFunc = (cache, { data }) => {
  ...
  const getGlobalPostsByIdUpdateFunction: MutationUpdateFunc = (
    cache,
    { data }
  ) => {
    let fromCache = cache.readQuery<GetGlobalPostByIdQuery>({
    ...
    if (
      ...
      cache.writeQuery<GetGlobalPostByIdQuery>({
        ...
        data: { getGlobalPostById: newCacheData }
      });

      cache.readQuery<GetGlobalPostByIdQuery>({
        query: GetGlobalPostByIdDocument,
        variables: {
          getpostinput: {
            postId: id
                ...
                <Button
                  ...
                  onClick={() => {
                    likesMutation({
                      ...
                      update: (cache, { data }) => {
                        isThisADynamicRoute === true
                          ? getGlobalPostsByIdUpdateFunction(cache, {
                              data
                            })
                          : globalPostsUpdateFunction(cache, { data });
                      }
                    });
                  }}
                >
